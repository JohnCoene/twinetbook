...
Below are the packages that make up in the _twinetverse_ as well as some explanation of what they bring to visualising Twitter interactions. Though considered a package the 'verse itself only comes with two functions, all the other functions we'll use in this book are part of the packages detailed below.
...
The graphTweets package [@R-graphTweets] lets you extract nodes and edges from tweets collected with rtweet. The package is fairly straighforward as it inludes just a few functions. However, it still enables you to build relatively complex graphs.
