...
net <- tl %>% 
  ...
  gt_edges(src, mentions_screen_name) %>% 
  gt_nodes() %>% 
  gt_collect()
```

Then we unpack the network, count the edges and prepare the data for sigmajs.
...
As you can see all the nodes (users) are linked to a single node, which is the user (\@jdatap) whose timeline we downloaded.
