...
We again collect tweets but feel free to use data from a previous chapter.

```{r, eval=TRUE}
# TK <- readRDS(file = "token.rds")
tweets <- search_tweets("#rstats filter:mentions", n = 2000, token = TK, include_rts = FALSE)
```
...
Now the actual visualisation, then again, we set `cumsum = FALSE`, and add a button to let you trigger the visualisation. before then you should see a blank canvas.
