...
sigmajs goes hand in hand with Shiny. As [stated in the wiki](https://github.com/jacomyal/sigma.js/wiki), sigma.js is "interactivity oriented" and therefore plays ideally in apps. Note that this chapter is also demonstrated online: [shiny.john-coene.com/sigmajs/](http://shiny.john-coene.com/sigmajs/).
...
simgajs lets you dynamically interact with the graph, that is, add/remove/customise without redrawing the whole graph: these functions are proxies and all end in `_p`. The package also lets you catch how the user interacts with your graph.

## Proxies

Below are some of the proxies sigmajs makes available to the user:
...
ui <- fluidPage(
  actionButton("getToken", "Get Token")
)
...
Now that we have a simple app going we can look at proxies. The layout looks a bit rubbish, we can add a proxy to let the user trigger the forceAtlas2 layout.
