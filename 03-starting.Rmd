...
Let's create a token, see the [packages](#pkgs) section to get setup.
...
Now that you have a token you are ready to download get Twitter data. Before we do so let's save our token; we're going to use it quite a lot and there is no need to create a new one every time you want to get Twitter data, the token is automatically refreshed by the underlying httr [@R-httr].
...
Now we can use the second package part of the _twinetverse_, graphTweets. Again, we'll function's arguments to default to get a simple graph. There's a lot more to the package which we'll get into later.
